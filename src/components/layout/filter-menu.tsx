 ...
 * [OUTPUT]: 对外提供 FilterMenu（分类单选 + 标签多选 AND 的合一筛选下拉）与 isListingPath()
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        aria-label={t("filter")}
        className={cn(
          "inline-flex h-10 shrink-0 items-center gap-1.5 rounded-full px-3.5 text-[15px] font-medium text-foreground outline-none transition-colors hover:bg-muted focus-visible:ring-3 focus-visible:ring-ring/30 data-[state=open]:bg-muted",
          pending && "opacity-70",
        )}
      >
        <span className="max-w-[9rem] truncate">{label}</span>
        {activeTags.length > 0 && (
          <span className="grid h-5 min-w-5 place-items-center rounded-full bg-foreground px-1 text-[11px] font-semibold text-background tabular-nums">{activeTags.length}</span>
        )}
